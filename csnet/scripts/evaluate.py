...
def init_logger(log: str = None):
    ...
    return logger, metricslogger, csvlogger
...
def main(args=None, running_as_script: bool = True):
    ...
    logger, metricslogger, csvlogger = init_logger(args.log)
    ...
    def out_callback(batch_index, chunk_index, out, ref_data, data, pbar, **kwargs): # Keep **kwargs or callback fails
        ...
        def format_csv(data, pred_data, ref_data, batch_index, chunk_index):
            try:
                ...
                if len(node_type)   > num_node_centers: node_type   = node_type  [node_centers]
                ...
                if uncertainty is not None and len(uncertainty) > num_node_centers: uncertainty = uncertainty[node_centers]
                if len(atom_fullname) > num_node_centers: atom_fullname = atom_fullname[node_centers.cpu().numpy()]
                dataset_raw_file_name = data[AtomicDataDict.DATASET_RAW_FILE_NAME][0]

                # Initialize lines list for CSV format
                lines = []
                if pbar.n == 0:
                    lines.append("batch,chunk,atom_fullname,atom_number,node_type,pred,ref,uncertainty_std,dataset_raw_file_name")
    
                for idx, (_atom_fullname, _atom_number, _node_type, _node_output) in enumerate(zip(atom_fullname, atom_number, node_type, node_output)):
                    _ref_node_output = ref_node_output[idx].item() if ref_node_output is not None else 0
                    _uncertainty = uncertainty[idx].abs().item() if uncertainty is not None else 0
                    lines.append(f"{batch_index:6},{chunk_index:4},{_atom_fullname:16},{_atom_number.item():6},{_node_type.item():6},{_node_output.item():10.4f},{_ref_node_output:10.4f},{_uncertainty:8.4f},{dataset_raw_file_name}")
            except:
                return ''
            # Join all lines into a single string for XYZ format
            return "\n".join(lines)

        csvlogger.info(format_csv(data, out, ref_data, batch_index, chunk_index))
