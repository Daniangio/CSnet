...
class EmbeddingGraphAttrs(GraphModuleMixin, torch.nn.Module):
    ...
    def __init__(
        ...
        super().__init__()
        if graph_input_num_types is None:
            raise Exception(f"Param 'graph_input_num_types' is None. This means that there are no graph inputs registered.")
    ...
    def forward(self, data: AtomicDataDict.Type) -> AtomicDataDict.Type:
        if AtomicDataDict.GRAPH_INPUT_TYPE_KEY in data and data.get(AtomicDataDict.GRAPH_ATTRS_KEY, None) is None:
