...
from csnet.network.allegro.nn._fc import ScalarMLPFunction, ExponentialScalarMLPFunction
from csnet.network.allegro import _keys
from csnet.network.allegro.nn._strided import Contracter, MakeWeightedChannels, Linear
from csnet.network.allegro.nn.cutoffs import cosine_cutoff, polynomial_cutoff

from csnet.network.mace.blocks import EquivariantProductBasisBlock
from csnet.network.mace.irreps_tools import reshape_irreps, inverse_reshape_irreps
...
@compile_mode("script")
class EdgeInteractionModule(GraphModuleMixin, torch.nn.Module):
    # saved params
    num_layers: int
    field: str
    inv_out_field: str
